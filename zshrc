...
function redacted-open-admin() {
  protocol=$(kubectl $([ ! -z "$2" ] && echo "--context redacted-cluster-$2") get secrets project-environment $([ ! -z "$1" ] && echo "--namespace redacted-$1") -o yaml | yq '.data.PROJECT_CONFIG' | base64 -d | yq '.project.protocol')
  hostname=$(kubectl $([ ! -z "$2" ] && echo "--context redacted-cluster-$2") get secrets project-environment $([ ! -z "$1" ] && echo "--namespace redacted-$1") -o yaml | yq '.data.PROJECT_CONFIG' | base64 -d | yq '.project.hostname')
  prefix_path=$(kubectl $([ ! -z "$2" ] && echo "--context redacted-cluster-$2") get secrets project-environment $([ ! -z "$1" ] && echo "--namespace redacted-$1") -o yaml | yq '.data.PROJECT_CONFIG' | base64 -d | yq '.network."prefix-path"')
  kubectl $([ ! -z "$2" ] && echo "--context redacted-cluster-$2") get secrets project-environment $([ ! -z "$1" ] && echo "--namespace redacted-$1") -o yaml | yq '.data.PROJECT_CONFIG' | base64 -d | yq '.authentication.inhouse.administrator.password' | copy
...
# Archive buckets for redacted platforms
function archive-buckets() {
  (gsutil label get gs://redacted-$2-$1-backups | grep -q archived_since) && echo "Bucket is already labelled" || gsutil label ch -l archived_since:$(date +%s) gs://redacted-$2-$1-backups
  (gsutil label get gs://redacted-$2-$1-storage | grep -q archived_since) && echo "Bucket is already labelled" || gsutil label ch -l archived_since:$(date +%s) gs://redacted-$2-$1-storage

  echo -e "\033[1;36mlatest backups by date:"
  gsutil ls -lah gs://redacted-$2-$1-backups/ | sort -k 2
  echo -e "\033[0m\n\n"

  echo "Changing default storage class"
  gsutil defstorageclass set ARCHIVE gs://redacted-$2-$1-backups
  gsutil defstorageclass set ARCHIVE gs://redacted-$2-$1-storage

  echo "Archiving all files..."
  gsutil -m rewrite -Ors ARCHIVE "gs://redacted-$2-$1-storage/"
  gsutil -m rewrite -Ors ARCHIVE "gs://redacted-$2-$1-backups/"

  echo "Removing users from $2-$1-backups Bucket's permission..."
  users=$(gsutil iam get gs://redacted-$2-$1-backups  | jq -r '.bindings[].members[] ' | sort | uniq | grep "user:")
  echo "${users}"
  echo "${users}" | xargs -r -I % gsutil iam ch -d % gs://redacted-$2-$1-backups
  echo "Removing users from $2-$1-storage Bucket's permission..."
  users=$(gsutil iam get gs://redacted-$2-$1-storage  | jq -r '.bindings[].members[] ' | sort | uniq | grep "user:")
  echo "${users}"
  echo "${users}" | xargs -r -I % gsutil iam ch -d % gs://redacted-$2-$1-storage
...
function use_common_cluster {
  gcloud beta compute ssh redacted-bastion \
