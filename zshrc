...
function redacted-open-admin() {
  ...
  kubectl $([ ! -z "$2" ] && echo "--context redacted-cluster-$2") get secrets project-environment $([ ! -z "$1" ] && echo "--namespace redacted-$1") -o yaml | yq '.data.PROJECT_CONFIG' | base64 -d | yq '.authentication.inhouse.administrator.password'
